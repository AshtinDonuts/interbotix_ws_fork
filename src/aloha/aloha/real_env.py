...
class RealEnv:
    ...
    def get_qpos(self):
        ...
        for name, bot in self.robots.items():
            if "follower" in name:
                ...
                # NOTE
                # Orignial implementation is incorrect \
                # It fetched gripper position in radians, but normalises \
                # using Linear units (m) \
    ...
    def get_gravity_torque(self):
        """
        Gather and concatenate gravity-induced torques for all follower robots' arms and grippers.

        Returns:
            np.ndarray: Array of concatenated gravity torques for arms and grippers.
            Format: [arm_joints (6), gripper (0), ...] per follower robot.
            Note: Gripper gravity torque is set to 0 as grippers don't have meaningful gravity torque.
        """
        # Initialize a list to hold the gravity torques for all arms and grippers
        gravity_torque_list = []

        # Iterate through all follower robots in the self.robots dictionary
        for name, bot in self.robots.items():
            if "follower" in name:
                # Get the gravity torque values for the arm
    ...
    def set_gripper_pose(self, robot_name: str, gripper_desired_pos_normalized: float, debug: bool) -> None:
        """
        ...
        # DEBUG: Print the desired gripper joint value if debug is True
        if debug:
            print(f"desired_gripper_joint: {desired_gripper_joint}")
    ...
    def get_observation(self):
        obs = collections.OrderedDict() # Q: Why OrderedDict?
        ## Each method below interfaces with InterbotixArmXSInterface
        ## which is where they actually access the Interbotix Core module
    ...
    def step(self, action, base_action=None, get_obs=True, debug=False):
        ...
        for name, robot in follower_robots.items():
            ...
            self.set_gripper_pose(name, bot_action[-1], debug)  # NOTE
            index += state_len
