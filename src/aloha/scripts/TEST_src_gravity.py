...
def main(args: dict) -> None:
    ...
    """
    gravity_compensation = args.get('get_gravity_torque', True)
        ...
        for leader_name, leader_bot in robots.items():
            if 'leader' in leader_name:
                ...
                if follower_bot:
                    ...
                    ##  DEC 29 - Computing Jacobian TEST
                    from typing import List
                    import numpy as np
                    follower_state_joints: List[float] = follower_bot.arm.get_joint_positions()
                    import modern_robotics as mr
                    J = mr.JacobianSpace(follower_bot.arm.robot_des.Slist, follower_state_joints)
                    assert isinstance(J, np.ndarray) and J.shape[0] == 6 and J.shape[1] == len(follower_state_joints), \
                        f"Jacobian shape is {J.shape}, expected (6, N)"
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        action='store_true',
        help='If set, get the gravity torques',
    )
    parser.add_argument(
        '-r', '--robot',
        required=True,
