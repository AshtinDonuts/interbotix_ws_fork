...
""" 
    Captures one episode
    To capture multiple episodes, use auto_record.sh
"""
...
def capture_one_episode(
    ...
    # JAN 9 - DISABLE OPENING GRIPPERS FOR SAFETY
    # move_grippers(
    ...
    # Get actual image dimensions from the first image if available
    ## TODO: my intuition says we can nest everything under the first statement as an if-conditional.
    image_shape = None
    if camera_names and not COMPRESS:
        ...
        if first_image is not None:
            image_shape = first_image.shape  # Should be (height, width, channels)
            ...
            for cam_name in camera_names:
                if COMPRESS:
                    ...
                    if image_shape is not None:
                        ...
                        # Fallback to default dimensions if no images available
...
if __name__ == "__main__":
    ...
    # Enable Gravity Torque
    # NOTE : AT THIS MOMENT, WE CANNOT USE BOTH GRAIVTY-TORQUE AND GRAVITY-COMPENSATION.
    # TODO  - fix incompatibility between gravity-torque and gravity-compensation
    parser.add_argument(
        "--gravity_torque",
        action="store_true",
